...
class ProductionRuntime:
    ...
    def _build_pdna_profile(self, repository_records):
        ...
        if not records:
            return {
                ...
                "reason": "No repository records available",
        ...
        for record in records:
            ...
            occurrences = hpi._coerce_int(record_dict.get("occurrences"))
            wins = hpi._coerce_int(record_dict.get("wins"))
            losses = hpi._coerce_int(record_dict.get("losses"))
            ...
            total_wins += wins
            total_losses += losses
            total_return += average_return * occurrences
            total_confidence += confidence * occurrences
            patterns.add(record_dict.get("pattern_dna"))

        if total_occurrences <= 0:
            return {
                "status": "INSUFFICIENT_HISTORY",
                "reason": "No occurrence history available",
                "historical_occurrences": 0,
            }

        historical_success_rate = float(total_wins) / total_occurrences if total_occurrences > 0 else 0.0
        historical_average_return = total_return / total_occurrences
        historical_confidence = total_confidence / total_occurrences
        pattern_maturity = hpi._build_pattern_maturity(total_occurrences)
        success_rates = [hpi._coerce_float(record.get("success_rate")) or 0.0 for record in records]
        pattern_stability = hpi._build_pattern_stability(success_rates)
        ...
        return {
            ...
            "reason": "Stock-specific historical PDNA profile generated from repository records",
            "symbols": sorted(set(record.get("symbol") for record in records if record.get("symbol"))),
            "pattern_dna": list(patterns),
    ...
    def run(self):
        ...
        pdna_profile = self._build_pdna_profile(repository_records)
