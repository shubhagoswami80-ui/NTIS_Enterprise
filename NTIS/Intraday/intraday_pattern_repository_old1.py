...
class IntradayPatternRepository:
    ...
    def _load_repository(self) -> pd.DataFrame:
        if self.repo_file.exists():
            try:
                ...
                if not df.empty:
                   if "Pattern_DNA" not in df.columns:
                       df["Pattern_DNA"] = ""

                if "Pattern_ID" not in df.columns:
                    df["Pattern_ID"] = ""

                df.to_csv(self.repo_file, index=False)
                return df
            except Exception:
                pass
    ...
    def get_or_create_pattern_id(self, symbol: str, fingerprint: str, sample_row: dict, trade_date: str = None) -> str:
        ...
        new_row = {
            ...
            "Pattern_DNA": str(sample_row.get("Pattern_DNA", "")),
            "Pattern_ID": str(sample_row.get("Pattern_ID", "")),
