...
def _live_timing_log(day: dict[str, Any], path: Path, stage_times: dict[str, float], total_seconds: float) -> None:
    """Persist bounded diagnostic timing for the last LIVE snapshot.

    Diagnostic metadata only: it does not participate in checkpoint selection,
    qualification, ranking, alert generation, or replay chronology. Keeping the
    latest sample in durable state lets a market-session stall be diagnosed
    without introducing a second log/state system.
    """
    samples = day.get("live_processing_timing", []) or []
    if not isinstance(samples, list):
        samples = []
    sample = {
        "source_file": str(path),
        "market_snapshot_time": parse_display_observation_timestamp(path).isoformat(),
        "filesystem_creation_time": parse_observation_timestamp(path).isoformat(),
        "process_snapshot_seconds": round(float(stage_times.get("process_snapshot", 0.0)), 3),
        "first_alert_seconds": round(float(stage_times.get("first_alert", 0.0)), 3),
        "retracement_seconds": round(float(stage_times.get("retracement", 0.0)), 3),
        "state_save_seconds": round(float(stage_times.get("state_save", 0.0)), 3),
        "total_seconds": round(float(total_seconds), 3),
        "recorded_at": datetime.now().isoformat(),
    }
    samples.append(sample)
    day["live_processing_timing"] = samples[-20:]
    day["last_live_processing_timing"] = sample
...
def parse_display_observation_timestamp(path: Path) -> datetime:
    """Return the market/report snapshot time for dashboard presentation.

    Daywise report filenames carry the represented market snapshot time as
    YYYY-MM-DD_HHMMSS.  Windows CreationTime remains authoritative for source
    chronology/checkpoint ordering and is intentionally not changed here.
    This presentation timestamp prevents a file copied/saved later from
    displaying its filesystem arrival time as the market snapshot time.
    """
    path = Path(path)
    match = re.search(r"(?P<date>\d{4}-\d{2}-\d{2})[_-](?P<time>\d{6})(?:\D|$)", path.stem)
    if match:
        try:
...
def _cached_day_folder_paths(root_str: str) -> tuple[str, ...]:
    """Return exact YYYY-MM-DD day folders under the configured source root."""
    ...
    for item in root.rglob("*"):
        try:
            if item.is_dir() and re.fullmatch(r"20\d{2}-\d{2}-\d{2}", item.name):
...
def _day_folders(source_root: Path, trading_date: str) -> tuple[Path, ...]:
    """Return every physical folder whose directory name exactly equals the trading date.

    The date folder may be nested arbitrarily below the configured screenshot
    root.  Filename dates are never used to establish day membership.
    """
    root = Path(source_root).expanduser()
    target = str(trading_date).strip()
    if not root.is_dir() or not re.fullmatch(r"20\d{2}-\d{2}-\d{2}", target):
        return ()
    found: list[Path] = []
    # The configured source root may itself be the active YYYY-MM-DD folder.
    # This is a valid deployment layout and must not be excluded merely because
    # rglob() only enumerates descendants.
    if root.name == target and root.is_dir():
        found.append(root)
    direct = root / target
    if direct.is_dir() and direct not in found:
        found.append(direct)
    try:
        for value in _cached_day_folder_paths(str(root)):
            candidate = Path(value)
            if candidate.name == target and candidate.is_dir() and candidate not in found:
                found.append(candidate)
    except OSError:
        pass
    return tuple(sorted(found, key=lambda p: str(p).lower()))


def _day_folder(source_root: Path, trading_date: str) -> Path | None:
    """Return the unique physical trading-day folder when one exists."""
    folders = _day_folders(source_root, trading_date)
    if not folders:
        return None
    if len(folders) == 1:
        return folders[0]
    # Multiple exact-date folders are an ambiguity. Prefer the folder that
    # actually contains Daywise files; if more than one does, fail closed
    # rather than silently mixing independent source trees.
    populated = []
    for folder in folders:
        try:
            if any(p.is_file() and not p.name.startswith("~$") for p in folder.rglob("Daywise_*.xlsx")):
                populated.append(folder)
        except OSError:
            continue
    if len(populated) == 1:
        return populated[0]
...
def _inventory_paths(source_root: Path, trading_date: str | None = None) -> list[Path]:
    """Return Daywise files only from exact physical day folders.

    For a selected date, all matching exact-date folders are considered only
    when there is no ambiguity. This protects source authenticity while still
    supporting nested year/month/day layouts.
    """
    root = Path(source_root).expanduser()
    if trading_date is not None:
        folders = _day_folders(root, trading_date)
        if not folders:
            return []
        if len(folders) > 1:
            populated = []
            ...
            if len(populated) != 1:
                return []
            folders = tuple(populated)
        paths: list[Path] = []
        try:
            for folder in folders:
                paths.extend(p for p in folder.rglob("Daywise_*.xlsx")
...
def _discover_sources(
    ...
    """Discover snapshots only inside the selected physical day folder."""
    root = Path(source_root or INTRADAY_SOURCE_ROOT).expanduser()
    files = _inventory_paths(root, trading_date)
    return sorted(
        [p for p in files if p.is_file()],
        key=lambda p: (parse_observation_timestamp(p), p.stat().st_mtime_ns, p.name.lower()),
    )


def _available_trading_dates(source_root: Path) -> list[str]:
    """Return only physical YYYY-MM-DD day folders that exist.

    If the configured source root is itself a day folder, expose that date as
    an available replay day as well.
    """
    root = Path(source_root).expanduser()
    dates: set[str] = set()
    if root.is_dir() and re.fullmatch(r"20\d{2}-\d{2}-\d{2}", root.name):
        dates.add(root.name)
    dates.update(Path(value).name for value in _cached_day_folder_paths(str(root)))
    return sorted(dates)
...
def _replay_cache_complete(value: Any) -> bool:
    if not isinstance(value, dict):
        return False
    if int(value.get("version", 0) or 0) != REPLAY_CACHE_VERSION:
        return False
    ...
    if not isinstance(snapshots, dict) or not snapshots:
        return False
    if not isinstance(timeline, pd.DataFrame) or not isinstance(manifest, list):
        return False
    if len(manifest) != len(snapshots):
        return False
    return all(isinstance(frame, pd.DataFrame) and "source_timestamp" in frame.columns for frame in snapshots.values())


def _source_manifest(sources: list[Path]) -> list[dict[str, str]]:
    ...
    for path in sources:
        try:
            manifest.append({
                ...
                "creation_timestamp": parse_observation_timestamp(path).isoformat(),
            })
...
def _replay_cache_matches_sources(value: Any, sources: list[Path]) -> bool:
    if not _replay_cache_complete(value):
        return False
    return value.get("source_manifest", []) == _source_manifest(sources)
...
def _store_replay_cache(
    ...
    value = {
        ...
        "source_count": int(len(snapshot_results)) if isinstance(snapshot_results, dict) else 0,
        "timeline": timeline,
        "point_in_time_cache": point_in_time_cache or {},
        "source_manifest": _source_manifest(sources or []),
    }
...
def _get_replay_cache(
    trading_date: str,
    sources: list[Path] | None = None,
) -> dict[str, Any]:
    # Every cache consumer is validated against the current physical source
    # chain. This prevents a previously generated cache from silently surviving
    # source deletion/addition or an accidental root-level file.
    expected_sources = sources if sources is not None else _discover_sources(trading_date)
    value = st.session_state.get(_cache_key(trading_date))
    if _replay_cache_complete(value) and _replay_cache_matches_sources(value, expected_sources):
        return value
    ...
    if not _replay_cache_complete(restored):
        return {}
    if not _replay_cache_matches_sources(restored, expected_sources):
        return {}
...
def _auto_process_new_snapshots(
    ...
    cache = _get_replay_cache(trading_date, sources)
    ...
    for path in new_paths:
        ...
        stage_times["process_snapshot"] = time.perf_counter() - process_started
        result = _attach_snapshot_metadata(result, path)
        timestamp = parse_observation_timestamp(path)

        first_alert_started = time.perf_counter()
        result = _update_first_alerts(
            state, trading_date, result, timestamp, first_alerts
        )
        stage_times["first_alert"] = time.perf_counter() - first_alert_started
        ...
        stage_times["retracement"] = time.perf_counter() - retracement_started
        ...
        stage_times["state_save"] = time.perf_counter() - save_started
        _live_timing_log(
            day,
            path,
            stage_times,
        ...
        # Timing is diagnostic metadata only. Persist it alongside the same
        # already-required state write; it never affects the checkpoint.
        save_state(state, STATE_JSON)
        state_changed_any = True

    timeline = pd.DataFrame(timeline_rows)
    _store_replay_cache(trading_date, cached_snapshots, timeline, sources=sources)
...
.replay-calendar-title{font-size:16px;font-weight:800;color:#0f172a;margin:2px 0 2px}
.replay-calendar-sub{font-size:11px;color:#64748b;margin-bottom:7px}
.replay-calendar-legend{font-size:10px;color:#64748b;margin:3px 0 8px;display:flex;align-items:center;gap:5px}
.replay-calendar-legend .dot{width:9px;height:9px;border-radius:50%;background:#172554;display:inline-block}
[data-testid="stVerticalBlock"]:has(.replay-calendar-title) button[kind="primary"]{background:#172554 !important;border-color:#172554 !important;color:#fff !important;font-weight:800 !important;min-height:2.3rem !important}
[data-testid="stVerticalBlock"]:has(.replay-calendar-title) button[kind="secondary"]{background:#f8fafc !important;border-color:#e2e8f0 !important;color:#cbd5e1 !important;min-height:2.3rem !important}

/* Replay availability calendar: compact, readable, and visually explicit. */
.replay-calendar-title{font-size:16px;font-weight:800;color:#0f172a;margin:2px 0 2px}
.replay-calendar-sub{font-size:11px;color:#64748b;margin-bottom:7px}
.replay-calendar-legend{font-size:10px;color:#64748b;margin:3px 0 8px;display:flex;align-items:center;gap:5px}
.replay-calendar-legend .dot{width:9px;height:9px;border-radius:50%;background:#172554;display:inline-block}
[data-testid="stVerticalBlock"]:has(.replay-calendar-title) [data-testid="stBaseButton-primary"]{background:#172554 !important;border-color:#172554 !important;color:#fff !important;font-weight:800 !important;min-height:2.35rem !important}
[data-testid="stVerticalBlock"]:has(.replay-calendar-title) [data-testid="stBaseButton-secondary"]{background:#f8fafc !important;border-color:#e2e8f0 !important;color:#cbd5e1 !important;min-height:2.35rem !important}
</style>
""",
...
def _ui_snapshot_signature(snapshot_results: dict[str, pd.DataFrame] | None) -> tuple[str, ...]:
    """Stable presentation-cache signature for the currently available snapshots.

    This is deliberately based on the existing source keys only. It is not a
    decision/evidence key and does not participate in engine calculations.
    """
    if not isinstance(snapshot_results, dict) or not snapshot_results:
        return ()
    return tuple(sorted(str(key) for key in snapshot_results.keys()))


def _cached_symbol_history_index(
    snapshot_results: dict[str, pd.DataFrame] | None,
    symbols: set[str] | None = None,
    cache_scope: str = "live",
) -> dict[str, list[pd.Series]]:
    """Reuse the diagnostic history index across UI-only fragment reruns.

    LIVE filter/selectbox interactions do not change the underlying snapshot
    chain, so rebuilding this index on every interaction is unnecessary. The
    cache is invalidated automatically when the available snapshot-key chain
    changes. This is presentation-only and leaves decision/evidence logic
    untouched.
    """
    wanted = tuple(sorted({str(v).strip().upper() for v in symbols or set() if str(v).strip()}))
    signature = (_ui_snapshot_signature(snapshot_results), wanted, str(cache_scope))
    cache = st.session_state.setdefault("ds_ui_history_cache", {})
    cached = cache.get(signature)
    if isinstance(cached, dict):
        return cached
    value = _build_symbol_history_index(snapshot_results, set(wanted))
    # Keep only the newest few presentation indexes; LIVE grows every cycle.
...
def _ui_diag_cache_get(scope: str, signature: str, name: str, builder):
    """Cache expensive presentation diagnostics for one rendered snapshot.

    Widget interactions frequently rerun the same fragment without changing
    the underlying row. Reusing these diagnostics prevents the dashboard from
    repeatedly rescanning historical snapshots merely to redraw a filter.
    """
    root = st.session_state.setdefault("ds_ui_diag_cache", {})
    scope_cache = root.setdefault(str(scope), {})
    key = f"{signature}|{name}"
    if key not in scope_cache:
        scope_cache[key] = builder()
    # Keep the per-scope cache bounded.
    if len(scope_cache) > 600:
        for old_key in list(scope_cache)[:200]:
            scope_cache.pop(old_key, None)
...
def _break_quality_diagnostic(
    ...
    history = (
        ...
        else _symbol_snapshot_history(snapshot_results, symbol)
    )
...
def _render_opportunity_cards(
    ...
    for rank, (_, row) in enumerate(candidates.head(limit).iterrows(), start=1):
        ...
        row_ts = str(row.get("source_timestamp", row.get("observation_timestamp", ""))).strip()
        diag_scope = str(st.session_state.get("ds_trading_date", "unknown"))
        diag_sig = f"{row_symbol}|{row_ts}|{_ui_snapshot_signature(snapshot_results)}"
        lifecycle = _ui_diag_cache_get(
            diag_scope, diag_sig, "lifecycle",
            lambda: _alert_lifecycle_diagnostic(row, snapshot_results),
        )
        ...
        if first == '—':
            try:
                ...
                pass
        sr=_sr_text(row)
        outlook=_ui_diag_cache_get(
            diag_scope, diag_sig, "outlook",
            lambda: _setup_outlook_diagnostic(row),
        )
        # Filter interactions must remain presentation-only. The detailed
        # retracement engine is available from the explicit audit button below;
        # cards use already-recorded durable lifecycle state when available.
        symbol_events = {}
        for event in state_day.get("retracement_events", []) or []:
            if isinstance(event, dict) and str(event.get("symbol", "")).strip().upper() == row_symbol:
                symbol_events[str(event.get("event", "")).upper()] = event
        retrace = (
            (state_day.get("retracement_alerts", {}) or {}).get(row_symbol, {})
            or (state_day.get("retracement_watches", {}) or {}).get(row_symbol, {})
...
def _render_live_queue(
    ...
    work['_first']=pd.to_datetime(work['first_alert_timestamp'],errors='coerce')
    # LIVE queue must never present an alert or source observation from beyond
    # the regular NSE cash-market session. This is a display integrity guard;
    # it does not mutate durable state or the frozen decision engine. Replay is
    # point-in-time and therefore deliberately retains its own historical rules.
    if not replay_mode:
        trading_date = str(st.session_state.get('ds_trading_date', '')).strip()
        try:
            session_date = datetime.strptime(trading_date, '%Y-%m-%d').date()
        except ValueError:
            session_date = None
        if session_date is not None:
            close_dt = datetime(session_date.year, session_date.month, session_date.day, MARKET_CLOSE_TIME[0], MARKET_CLOSE_TIME[1])
            work = work.loc[~((work['_first'].dt.date == session_date) & (work['_first'] > close_dt))].copy()
            source_col = 'source_timestamp' if 'source_timestamp' in work.columns else ('observation_timestamp' if 'observation_timestamp' in work.columns else None)
            if source_col:
                work['_source_ts'] = pd.to_datetime(work[source_col], errors='coerce')
                work = work.loc[~((work['_source_ts'].dt.date == session_date) & (work['_source_ts'] > close_dt))].copy()
    ...
    for _,row in work.iterrows():
        ...
        diag_scope = str(st.session_state.get("ds_trading_date", "unknown"))
        diag_sig = f"{row_symbol}|{row_ts}|{_ui_snapshot_signature(snapshot_results)}"
        setup_q=_ui_diag_cache_get(
            diag_scope, diag_sig, "readiness",
            lambda: _setup_readiness_diagnostic(row, snapshot_results, history_by_symbol),
        )
        outlook_q=_ui_diag_cache_get(
            diag_scope, diag_sig, "outlook",
            lambda: _setup_outlook_diagnostic(row),
        )
        lifecycle_q=_ui_diag_cache_get(
            diag_scope, diag_sig, "lifecycle",
            lambda: _alert_lifecycle_diagnostic(row,snapshot_results),
        )
...
def _render_processing_output(
    ...
    with st.expander(
        f"Processing Output • {parse_display_observation_timestamp(source_path):%H:%M:%S} • latest processed snapshot",
        expanded=False,
    ):
        c1, c2, c3, c4 = st.columns(4)
        ...
        timing_sample = (
            (load_state(STATE_JSON).get(STATE_KEY, {}).get(
                str(st.session_state.get("ds_trading_date", "")), {}
            ) or {}).get("last_live_processing_timing", {})
        )
        if timing_sample:
            st.caption(
                "Last LIVE timing • "
                f"engine {float(timing_sample.get('process_snapshot_seconds', 0.0)):.2f}s • "
                f"retracement {float(timing_sample.get('retracement_seconds', 0.0)):.2f}s • "
                f"save {float(timing_sample.get('state_save_seconds', 0.0)):.2f}s • "
                f"total {float(timing_sample.get('total_seconds', 0.0)):.2f}s"
            )
        ...
        processing_symbols = {
            str(v).strip().upper()
            for v in display_source.get('symbol', pd.Series(dtype=str)).tolist()
            if str(v).strip()
        }
        processing_history = _cached_symbol_history_index(
            snapshot_results, processing_symbols, cache_scope='processing'
        ) if isinstance(snapshot_results, dict) and snapshot_results else {}

        def _cached_break(row: pd.Series) -> dict[str, Any]:
            symbol = str(row.get('symbol', '')).strip().upper()
            ts = str(row.get('source_timestamp', row.get('observation_timestamp', ''))).strip()
            signature = f"{symbol}|{ts}|{_ui_snapshot_signature(snapshot_results)}"
            return _ui_diag_cache_get(
                str(st.session_state.get('ds_trading_date', 'unknown')),
                signature,
                'processing_break_quality',
                lambda: _break_quality_diagnostic(
                    row, snapshot_results, processing_history
                ),
            )

        display_source['_setup_readiness'] = display_source.apply(
            lambda row: _ui_diag_cache_get(
                str(st.session_state.get('ds_trading_date', 'unknown')),
                f"{str(row.get('symbol','')).strip().upper()}|{str(row.get('source_timestamp', row.get('observation_timestamp',''))).strip()}|{_ui_snapshot_signature(snapshot_results)}",
                'processing_readiness',
                lambda: _setup_readiness_diagnostic(row, snapshot_results, processing_history),
            ).get('readiness', 0),
            axis=1,
        )
        display_source['_break_confirmation'] = display_source.apply(
            lambda row: _cached_break(row).get('sustain_label', '—'),
            axis=1,
        )
        display_source['_diagnostic_quality'] = display_source.apply(
            lambda row: _cached_break(row).get('quality', 0),
            axis=1,
        )
        ...
        if columns:
            ...
            if "Time" in output.columns and "Stock" in output.columns:
                def _display_market_time(row: pd.Series) -> str:
                    raw_path = str(row.get("source_path", row.get("source_file", ""))).strip()
                    if raw_path:
                        try:
                            return parse_display_observation_timestamp(Path(raw_path)).strftime("%H:%M:%S")
                        except Exception:
                            pass
                    return str(row.get("Time", "—"))
                display_time_source = output_source.copy()
                output["Time"] = display_time_source.apply(_display_market_time, axis=1).values
            output["Reason"] = output_source.apply(
                lambda row: _cached_break(row).get(
                    "reason", "Existing engine evidence only"
                ),
                axis=1,
            ).values
            ...
            with st.expander(
                ...
                audit_symbols = {
                    str(v).strip().upper()
                    for v in audit_source.get('symbol', pd.Series(dtype=str)).tolist()
                    if str(v).strip()
                }
                audit_history = _cached_symbol_history_index(
                    snapshot_results, audit_symbols, cache_scope='processing_full'
                ) if isinstance(snapshot_results, dict) and snapshot_results else {}

                def _cached_audit_break(row: pd.Series) -> dict[str, Any]:
                    symbol = str(row.get('symbol', '')).strip().upper()
                    ts = str(row.get('source_timestamp', row.get('observation_timestamp', ''))).strip()
                    signature = f"{symbol}|{ts}|{_ui_snapshot_signature(snapshot_results)}"
                    return _ui_diag_cache_get(
                        str(st.session_state.get('ds_trading_date', 'unknown')),
                        signature,
                        'full_audit_break_quality',
                        lambda: _break_quality_diagnostic(
                            row, snapshot_results, audit_history
                        ),
                    )

                audit_source['_diagnostic_quality'] = audit_source.apply(
                    lambda row: _cached_audit_break(row).get('quality', 0),
                    axis=1,
                )
...
def _render_retracement_lifecycle(
    ...
    qualified_result = _rank(result)
    if qualified_result is None or qualified_result.empty:
        st.info("Retracement / re-entry audit loaded. No qualified lifecycle opportunities are recorded for this snapshot.")
        return
...
def _render_current_result(
    ...
    candidates = _rank(result)
    live_ui_scope = (
        f"{lifecycle_trading_date or st.session_state.get('ds_trading_date', 'unknown')}"
        f"|{snapshot_label}"
    )
    history_by_symbol = (
        _cached_symbol_history_index(
            snapshot_results,
            {
                str(v).strip().upper()
            ...
            cache_scope="replay" if lifecycle_replay else "live",
        )
    ...
    if lifecycle_replay:
        ...
        if st.session_state.get(f"{widget_key_prefix}show_replay_evolution_{replay_scope}", False):
            ...
            if isinstance(timeline, pd.DataFrame) and not timeline.empty:
                ...
                st.info("LIVE Stock Evolution loaded. No decision-evolution events are recorded for this LIVE state yet.")
        return
    ...
    if load_lifecycle:
        ...
        st.session_state[f"{widget_key_prefix}audit_notice_{live_scope}"] = "LIVE retracement / re-entry audit requested."
    if load_evolution:
        st.session_state[f"{widget_key_prefix}show_live_evolution_{live_scope}"] = True
        st.session_state[f"{widget_key_prefix}audit_notice_{live_scope}"] = "LIVE intraday stock evolution requested."
    audit_notice = st.session_state.get(f"{widget_key_prefix}audit_notice_{live_scope}")
    if audit_notice:
        st.caption(audit_notice)

    if st.session_state.get(f"{widget_key_prefix}show_live_lifecycle_{live_scope}", False):
        # If the user explicitly requests the LIVE retracement audit, hydrate
        # the read-only chronological cache on demand so
    ...
    # decision rule.
    if st.session_state.get(f"{widget_key_prefix}show_live_evolution_{live_scope}", False):
        if not isinstance(timeline, pd.DataFrame) or timeline.empty:
            st.info("LIVE Stock Evolution loaded. No decision-evolution events are recorded for this LIVE state yet.")
            return
...
if hasattr(st, "fragment"):
    ...
    def _live_auto_panel(source_root: Path, trading_date: str, rollover_fallback: bool = False) -> None:
        ...
        try:
            ...
            # A completed batch updates the shared LIVE render state. The separate
            # decision fragment polls that state; no full-app rerun is required.
            ...
            # LIVE FEED is a market-session process. After the NSE cash session
            # closes, newly arriving/post-market files must never advance LIVE
            # durable state or become the LIVE alert context. The last complete
            # market-session state remains authoritative until the next session.
            ...
            if rollover_fallback:
                ...
                if rollover_initial_render:
                    ...
                    latest, timeline, state_changed_any = _auto_process_new_snapshots(
                        sources, trading_date, max_batch=1
                    )
                    persisted_source = ""
                    persisted_timestamp = ""
                ...
                # During the live market session, the fragment checks frequently
                # but ordinary LIVE processing keeps its established 5-minute
                # cadence when no backlog exists.
                ...
                if backlog_pending and initial_live_render:
                    ...
                    latest, timeline, state_changed_any = _auto_process_new_snapshots(
                        sources,
                        trading_date,
                        # Bound each LIVE cycle to one snapshot so a slow
                        # snapshot cannot block the dashboard for a long batch.
                        max_batch=1,
                    )
                ...
                # Market closed / Auto-update OFF: NEVER rebuild the full day merely
                # to render the LIVE panel.  This is a presentation/restore path,
                # not a replay-generation path.  Rebuilding all sources here was
                # the confirmed cause of the 45+ second startup block observed
                # after market close.  Restore the durable last-complete LIVE
                # state; historical day generation belongs exclusively to Replay.
                restored_closed = _restore_last_complete_state(trading_date)
                if restored_closed is not None:
                    latest, timeline, persisted_source, persisted_timestamp = restored_closed
                else:
                    # No durable LIVE state exists yet (e.g. before the first
                    # decision-bearing snapshot).  Use only the latest source as a
                    # lightweight view rather than rebuilding the complete day.
                    latest, timeline = _load_day_for_snapshot_view(
                        sources[-1:], trading_date
                    )[:2]
                    persisted_source = ""
                    persisted_timestamp = ""
            ...
            # The presentation/audit cache is read-only and is useful after
            # market close as well. It must not be used to advance LIVE state.
            ...
            # The separate LIVE decision fragment polls this state and redraws the
            # decision board without restarting LIVE catch-up or the full dashboard.
        except Exception as exc:
...
def _build_replay_day_in_memory(
    ...
    if selected_index is None:
        _store_replay_cache(
            trading_date, snapshots, timeline,
...
def _render_available_replay_calendar(available_dates: list[str]) -> str:
    """Render a compact popover calendar with data-available days highlighted."""
    if not available_dates:
        return ""
    available = {str(v) for v in available_dates}
    selected = str(st.session_state.get("ds_replay_date", available_dates[-1]))
    current = st.session_state.get("ds_replay_calendar_month", "")
    try:
        month_anchor = datetime.strptime(str(current), "%Y-%m").date().replace(day=1)
    except ValueError:
        try:
            month_anchor = datetime.strptime(selected, "%Y-%m-%d").date().replace(day=1)
        except ValueError:
            month_anchor = datetime.strptime(available_dates[-1], "%Y-%m-%d").date().replace(day=1)
    st.session_state["ds_replay_calendar_month"] = month_anchor.strftime("%Y-%m")

    # The calendar is intentionally a popover: it keeps the replay controller
    # compact while retaining a real month view and explicit availability state.
    with st.popover(f"📅  {selected}", use_container_width=True):
        st.markdown('<div class="replay-calendar-title">Replay Calendar</div>', unsafe_allow_html=True)
        st.markdown('<div class="replay-calendar-sub">Dark dates = data available</div>', unsafe_allow_html=True)
        nav_prev, nav_title, nav_next = st.columns([1, 4, 1])
        with nav_prev:
            if st.button("‹", key="ds_replay_cal_prev", use_container_width=True):
                month_anchor = (month_anchor.replace(day=1) - timedelta(days=1)).replace(day=1)
                st.session_state["ds_replay_calendar_month"] = month_anchor.strftime("%Y-%m")
                st.rerun(scope="fragment")
        with nav_title:
            st.markdown(f"<div style='text-align:center;font-weight:800;padding-top:7px'>{month_anchor:%B %Y}</div>", unsafe_allow_html=True)
        with nav_next:
            if st.button("›", key="ds_replay_cal_next", use_container_width=True):
                month_anchor = (month_anchor.replace(day=28) + timedelta(days=4)).replace(day=1)
                st.session_state["ds_replay_calendar_month"] = month_anchor.strftime("%Y-%m")
                st.rerun(scope="fragment")

        weekdays = ["M", "T", "W", "T", "F", "S", "S"]
        cols = st.columns(7, gap="small")
        for col, label in zip(cols, weekdays):
            col.markdown(f"<div style='text-align:center;font-size:10px;font-weight:800;color:#64748b'>{label}</div>", unsafe_allow_html=True)
        first_weekday, days_in_month = calendar.monthrange(month_anchor.year, month_anchor.month)
        cells = [None] * first_weekday + list(range(1, days_in_month + 1))
        while len(cells) % 7:
            cells.append(None)
        for offset in range(0, len(cells), 7):
            cols = st.columns(7, gap="small")
            for col, day_num in zip(cols, cells[offset:offset + 7]):
                if day_num is None:
                    col.write("")
                    continue
                value = date(month_anchor.year, month_anchor.month, day_num).strftime("%Y-%m-%d")
                enabled = value in available
                if col.button(
                    str(day_num),
                    key=f"ds_replay_cal_day_{value}",
                    disabled=not enabled,
                    type="primary" if enabled else "secondary",
                    use_container_width=True,
                ):
                    st.session_state["ds_replay_date"] = value
                    st.session_state["ds_replay_selected_label"] = ""
                    st.session_state.pop("ds_replay_point_in_time", None)
                    st.rerun(scope="fragment")
        st.caption("Dark = source data available  •  Light = no source data")
    return str(st.session_state.get("ds_replay_date", selected))
    ...
    def _replay_panel() -> None:
        with st.expander("INTRADAY SNAPSHOT / REPLAY", expanded=False):
            if not available_dates:
                ...
                replay_date = str(
                    st.session_state.get("ds_replay_date", available_dates[-1])
                )

                calendar_col, controls_col = st.columns([1.25, 1.75], gap="large")
                with calendar_col:
                    replay_date = _render_available_replay_calendar(available_dates)
                with controls_col:
                    try:
                    ...
                    replay_day_cache = _get_replay_cache(replay_date, replay_sources)
                    replay_cache_ready = (
                        _replay_cache_complete(replay_day_cache)
                        and int(replay_day_cache.get("source_count", 0) or 0) == len(replay_sources)
                        and all(
                            _source_key(p) in replay_day_cache.get("snapshots", {})
                            for p in replay_sources
                        )
                    ) if replay_sources else False
                    st.caption(
                        "Historical day cache: READY — no data reprocessing on snapshot selection."
                        if replay_cache_ready
                        else "Historical day cache: not prepared — first View Snapshot will prepare this trading day once."
                    )
                    ...
                    if not replay_sources:
                        ...
                        submit_replay = st.button(
                            "View Snapshot" if replay_cache_ready else "Prepare Day & View Snapshot",
                            type="primary",
                            use_container_width=True,
                            key="ds_replay_view_snapshot",
                        )
                ...
                if replay_requested:
                    try:
                        ...
                        if not complete:
                            ...
                            live_cache = _get_replay_cache(replay_date, replay_sources)
                            live_snapshots = (
                                live_cache.get("snapshots", {})
                                if isinstance(live_cache, dict)
                                else {}
                            ...
                            historical_cache_usable = (
                                ...
                                and int(live_cache.get("source_count", 0) or 0) == len(replay_sources)
                                and all(_source_key(p) in live_snapshots for p in replay_sources)
                                and historical_key in live_snapshots
                            )
                            # TODAY'S replay can use the already-processed LIVE prefix
                            # when the selected historical snapshot itself has been
                            # processed. This avoids rebuilding the whole day merely
                            # to inspect an earlier point while LIVE is still catching up.
                            partial_live_cache_usable = False
                            if not historical_cache_usable and historical_key in live_snapshots:
                                cached_selected = live_snapshots.get(historical_key)
                                cached_selected_ts = _replay_frame_timestamp(cached_selected)
                                if cached_selected_ts is not None:
                                    durable_live_state = load_state(STATE_JSON).get(STATE_KEY, {}).get(
                                        replay_date, {}
                                    ) or {}
                                    durable_live_ts = pd.to_datetime(
                                        durable_live_state.get(
                                            "last_processed_observation_timestamp", ""
                                        ),
                                        errors="coerce",
                                    )
                                    if pd.notna(durable_live_ts) and cached_selected_ts <= pd.Timestamp(durable_live_ts):
                                        partial_live_cache_usable = True
                            if historical_cache_usable or partial_live_cache_usable:
                                ...
                                if not isinstance(point_cache, dict):
                                    point_cache = {}
                                # LIVE processing stores actual source frames with their
                                # persisted lifecycle attributes. Index those frames for
                                # replay without invoking the decision engine again.
                                if partial_live_cache_usable:
                                    for cached_key, cached_frame in live_snapshots.items():
                                        if cached_key in point_cache:
                                            continue
                                        cached_ts = _replay_frame_timestamp(cached_frame)
                                        if cached_ts is None or cached_ts > pd.to_datetime(
                                            cached_selected_ts, errors="coerce"
                                        ):
                                            continue
                                        lifecycle_attrs = cached_frame.attrs.get(
                                            "replay_lifecycle_events", {}
                                        )
                                        if not isinstance(lifecycle_attrs, dict):
                                            lifecycle_attrs = {}
                                        point_cache[cached_key] = {
                                            "source_timestamp": cached_ts.isoformat(),
                                            "result": cached_frame,
                                            "lifecycle_events": dict(lifecycle_attrs),
                                        }
                                ...
                                with st.spinner(
                                    f"Preparing historical day {replay_date} once — building point-in-time snapshots…"
                                ):
                                ...
                                st.success(
                                    f"Historical day {replay_date} prepared. Future snapshot selections will use the saved day cache."
                                )
                                view_cache = _get_replay_view_cache(
                                    replay_date, None
                                )
