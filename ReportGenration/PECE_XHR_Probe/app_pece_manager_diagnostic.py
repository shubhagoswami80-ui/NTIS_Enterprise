...
CFG = ROOT.parent / "reports.json"
PROFILE = ROOT.parent / "browser_profile"
LOGS = ROOT / "logs"
...
from pece_batch220_final_diagnostic import run_full_220
                    ...
                    while True:
                        ...
                        if command == "pece_full_220":
                            try:
                                ...
                                self.set(
                                    message="PE/CE 220 batch started...",
                                )
                                result = run_full_220(
                                    context,
                                    ROOT / "pece_xhr_probe",
                                    status_callback=lambda msg: self.set(message=msg),
                                    concurrency=6,
                                )
                                self.set(
                                    message=(
                                        ...
                                        f"{result.get('completed_count', 0)} successful"
                                    )
                                )
                            except Exception as error:
                                self.set(message=f"PE/CE failed: {error}")
                            continue
...
st.subheader("PE/CE 220 Diagnostic — Final Native XHR")
if st.button(
    "RUN PE/CE ALL 220 STOCKS — FINAL NATIVE XHR",
    use_container_width=True,
    disabled=status["running"],
):
