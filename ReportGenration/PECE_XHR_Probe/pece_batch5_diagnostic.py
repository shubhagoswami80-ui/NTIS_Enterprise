...
def discover_batch5(context,production_page,output_root):
    root=Path(output_root)
    out=root/f"batch5_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        if len(symbols)<5: raise RuntimeError(f"Only {len(symbols)} usable symbols found")
        results=[]
        for sym in symbols:
            before_n=len(events)
