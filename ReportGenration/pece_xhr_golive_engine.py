        ...
        try:
            if path.suffix.lower() == ".csv":
                ...
                with path.open(encoding="utf-8-sig", newline="") as fh:
                    values = {str(r.get("Symbol", "")).strip().upper() for r in csv.DictReader(fh)}
            else:
                data = json.loads(path.read_text(encoding="utf-8"))
                items = data.get("symbols", data) if isinstance(data, dict) else data
                values = {str(x.get("value", "") if isinstance(x, dict) else x).strip().upper() for x in items}
            values.discard("")
            values -= INDEX_SYMBOLS
            if values:
...
def _apply_daily_stock_filter(discovered, trace):
    cache = trace / "SYMBOL_UNIVERSE_DAILY.json"
    today = time.strftime("%Y-%m-%d")
    cached = None
    if cache.exists():
        try:
            obj = json.loads(cache.read_text(encoding="utf-8"))
            if obj.get("date") == today and isinstance(obj.get("symbols"), list):
                cached = {str(x).upper() for x in obj["symbols"]}
        except Exception:
            cached = None
    if cached is None:
        allowed, source = _load_project_symbols()
        if allowed is None:
            eligible = [s for s in discovered if str(s.get("value", "")).upper() not in INDEX_SYMBOLS]
            source = "iCharts_discovery_without_project_intersection"
        else:
            eligible = [s for s in discovered if str(s.get("value", "")).strip().upper() in allowed]
        cached = {str(s.get("value", "")).strip().upper() for s in eligible}
        _write_json(cache, {"date": today, "source": source, "symbols": sorted(cached), "count": len(cached)})
    eligible = [s for s in discovered if str(s.get("value", "")).strip().upper() in cached]
    return eligible, {"applied": True, "cache": str(cache), "date": today, "eligible_count": len(eligible), "discovered_count": len(discovered), "excluded_count": len(discovered)-len(eligible)}
...
def validate_payload(body: str):
    ...
    # An empty aaData response is a legitimate "no data available" result.
    # It must not invalidate or discard data returned for other symbols.
    if not data:
        return True, "no_data_available", 0, obj
    if not all(isinstance(row, list) for row in data):
        return False, "aaData_contains_non_list_row", 0, obj

    # iCharts may return rows with optional/unpopulated columns.  Do not reject
    # an otherwise parseable response merely because its width is not 31.
    # Preserve the observed width for diagnostics; downstream normalization
    # should represent missing fields as blank/NaN, never as numeric zero.
...
def _map_source_row(source_row):
    # Dry-run compatibility mode:
    # iCharts may return rows with omitted/unpopulated trailing fields.
    # Preserve the row and represent missing source fields as blank values.
    # The native mapping still uses the established 31-field source schema.
    if not isinstance(source_row, list):
        raise ValueError("PE/CE source row must be a list")

    observed_width = len(source_row)
    if observed_width < 31:
        source_row = list(source_row) + [""] * (31 - observed_width)
    elif observed_width > 31:
        # Keep the established source schema; ignore unexpected extra fields.
        source_row = list(source_row[:31])
...
def _relocate_legacy_day_files(day_dir: Path, trace: Path):
    """Move legacy audit/control files out of the report directory.

    Existing Excel reports remain in the date directory. Only non-Excel
    control, manifest, JSON, and text files are relocated.
    """
    ...
    for item in day_dir.iterdir():
        if not item.is_file():
            continue
        if item.suffix.lower() in {".xlsx", ".xls", ".xlsm", ".csv"}:
            continue
        target = trace / item.name
        if target.exists():
            target = trace / f"legacy_{int(time.time() * 1000)}_{item.name}"
        try:
            item.replace(target)
        except OSError:
            # A legacy file must never stop the data acquisition cycle.
            pass
...
def run_pece_xhr_batch(context, output_root, job, status_callback=None):
    ...
    _relocate_legacy_day_files(day_dir, trace)
    ...
    request_gap_ms = max(250, min(20000, int(job.get("request_start_gap_ms", 750))))
    ...
    jitter_min_ms = max(0, min(15000, int(job.get("request_jitter_min_ms", 250))))
    jitter_max_ms = max(jitter_min_ms, min(20000, int(job.get("request_jitter_max_ms", 750))))
    ...
    timeout_ms = max(
        ...
        min(
            30000,
            int(float(job.get("request_timeout_seconds", 15)) * 1000),
        ),
    )
    ...
    manifest = {
        ...
        "source_width_policy": "pad_missing_fields_for_dry_run",
    ...
    try:
        ...
        for s in symbols:
            if s["value"] not in seen:
                ...
                unique.append(s)
        symbols = unique
        symbols, universe_info = _apply_daily_stock_filter(symbols, trace)
        manifest["symbol_universe_filter"] = universe_info
