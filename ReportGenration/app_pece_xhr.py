...
from pece_xhr_engine import XHRReportEngine, XHRReportError
...
# ---- Generic iCharts XHR/API transport ---------------------------------
def _pece_xhr_download(self, playwright, context, page, job):
    if str(job.get("action", "")).strip().casefold() != "xhr":
        return _PECE_ORIGINAL_DOWNLOAD(self, playwright, context, page, job)
    try:
        engine = XHRReportEngine(logger=self.log)
        raw_path, latest_path, row_count = engine.run(context, page, job)
        self.set(last_download=f"{job['name']} | {latest_path}", message=f"{job['name']}: XHR snapshot completed ({row_count} rows).")
        self.log(f"{job['name']}: XHR latest -> {latest_path}")
    except Exception as error:
        self.log(f"{job['name']}: XHR download failed: {error}")
    return context, page

_PECE_ORIGINAL_DOWNLOAD = Manager.download
Manager.download = _pece_xhr_download
