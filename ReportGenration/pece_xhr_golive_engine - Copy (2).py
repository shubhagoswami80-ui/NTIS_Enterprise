...
def validate_payload(body: str):
    ...
    width = len(data[0])
    if width != 31:
        return False, f"unexpected_source_width_{width}", 0, obj
...
def _map_source_row(source_row):
    if not isinstance(source_row, list) or len(source_row) != 31:
        raise ValueError(
            f"PE/CE source row must contain exactly 31 fields; got "
            f"{len(source_row) if isinstance(source_row, list) else 'non-list'}"
        )
...
def run_pece_xhr_batch(context, output_root, job, status_callback=None):
    ...
    try:
        ...
        # Publish every discovered symbol. Valid zero values remain valid.
        # Symbols without a usable response stay in the main Data sheet with
        # blank report fields; their details remain available in Status/Run.
        ...
        # Preserve the complete discovered symbol list in Data. For a failed
        # symbol, add one blank row rather than removing the symbol entirely.
        existing_symbols = {str(row.get("Symbol", "")) for row in rows}
        for result in ordered:
            symbol = str(result.get("symbol", ""))
            if not result.get("valid") and symbol not in existing_symbols:
                blank = {column: "" for column in REPORT_HEADERS}
                blank.update({
                    "Symbol": symbol,
                    "Snapshot": cycle_id,
                    "Response_Row": "",
        ...
        if manifest["failed_count"] == 0:
            ...
            manifest["publish_reason"] = (
                "All discovered symbols have valid payloads."
            )
        else:
            manifest["status"] = "completed_with_gaps"
            manifest["integrity_gate"] = "PARTIAL"
            manifest["publish_reason"] = (
                "Published the complete discovered symbol list. Symbols with "
                "unavailable responses have blank Data fields and are listed "
                "in Status/missing_symbols for diagnostics and same-scan retry."
            )
