...
async def run_optionchain_end_to_end_collector(
    ...
    # Phase 1: acquire exact request contracts. Sequential by design so the
    # first benchmark does not introduce browser-state concurrency.
    captures = []
    capture_failures = []
    for symbol in symbols:
        try:
            captures.append(
                await _capture_one(context, symbol, timeout_seconds)
            )
        except Exception as exc:
