...
def _latest_workbook() -> tuple[Path | None, str | None]:
    ...
    latest = max((Path(p) for p in files), key=_source_timestamp)
    observed = _source_timestamp(latest).strftime("%d %b %Y, %H:%M:%S")
...
def _source_dates() -> list[str]:
    try:
        ...
        return []
    return sorted({_source_timestamp(p).date().isoformat() for p in files}, reverse=True)
...
def _visible_frame(df: pd.DataFrame, historical: bool = False) -> pd.DataFrame:
    columns = [
    ...
    if "observation_timestamp" in out.columns:
        out = out.rename(columns={"observation_timestamp": "TRIGGER TIMESTAMP"})
    if "symbol" in out.columns:
        out = out.rename(columns={"symbol": "SYMBOL"})
    if "direction" in out.columns:
        out = out.rename(columns={"direction": "DIRECTION"})
    return out
...
st.markdown(
    '<div class="section-card"><div class="section-title">↻ Controlled Replay</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="muted">Replay reads ONLY the configured Daywise source repository. '
    'It rebuilds the selected day chronologically and records the exact source observation '
    'timestamp for each first breakout trigger. No source workbook is modified.</div>',
    unsafe_allow_html=True,
)

replay_dates = _source_dates()
if not replay_dates:
    st.info("No Daywise source sessions are available for replay.")
else:
    replay_date = st.selectbox(
        "Replay Trading Date",
        replay_dates,
        format_func=lambda x: pd.Timestamp(x).strftime("%d %b %Y"),
        key="controlled_replay_date",
    )
    r1, r2 = st.columns(2)
    with r1:
        replay_one = st.button("↻ Replay Selected Day", width="stretch")
    with r2:
        replay_all = st.button("↻ Replay All Available Days", width="stretch")
    ...
    if replay_one:
        try:
            ...
            st.success(
                ...
                f"Source range {result['first_timestamp'] or '—'} → {result['last_timestamp'] or '—'}."
            )
            st.rerun()
        except Exception as exc:
    ...
    if replay_all:
        try:
            ...
            total_files = sum(int(r.get("files", 0)) for r in results)
            total_events = sum(int(r.get("events", 0)) for r in results)
            st.success(
                f"Replayed {len(results)} trading days / {total_files} source files; "
                f"{total_events} first-breakout events rebuilt."
            )
            st.rerun()
        except Exception as exc:
...
with c2:
    st.markdown(
        ...
        <div class="metric-card">
            ...
            <div class="metric-detail">Source observation time (filename when available)</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
if today_events.empty:
    ...
    visible = _visible_frame(today_events)
    st.dataframe(
        visible,
        width="stretch",
        hide_index=True,
...
if show_history:
    ...
    if events.empty or "trading_date" not in events.columns:
        ...
        visible = _visible_frame(historical, historical=True)
        st.dataframe(
            visible,
            width="stretch",
            hide_index=True,
...
st.markdown(
    ...
    <div class="footer">
        ...
        <span>
            ...
            &nbsp;&nbsp;
            Version: v1.1.0 • controlled replay + source timestamps
            &nbsp;&nbsp;
