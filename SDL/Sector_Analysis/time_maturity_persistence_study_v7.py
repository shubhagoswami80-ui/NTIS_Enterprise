...
def main():
    ...
    out=intel/"time_maturity_persistence_study_v7"; out.mkdir(parents=True,exist_ok=True)
    ...
    summary={"status":"FAILED","cache_only":True,"raw_source_scan":False,"output_dir":str(out),
             ...
             "maturity_cutoffs":list(CUTS),"primary_outcome":"existing reached_0.5x from multi_window_outcomes.csv (alias-safe column normalization)",
    ...
    for required in ["orb_minutes","symbol","trade_date","anchor_time","orb_direction"]:
    ...
    outcome_col=col(m,["reached_0.5x","reached_0_5x","reached 0.5x"] )
    if outcome_col is None:
        summary["reason"]="Missing required outcome field: reached_0.5x / reached_0_5x"
        (out/"time_maturity_summary.json").write_text(json.dumps(summary,indent=2)); print(json.dumps(summary,indent=2)); return 3
    m["_reached_0_5x"]=m[outcome_col].astype(str).str.strip().str.lower().isin(["true","1","yes","y"])
    ...
    for _,e in m.iterrows():
        ...
        for label,cut in CUTS.items():
            ...
            rec={"orb_minutes":e["orb_minutes"],"symbol":e["_symbol"],"trade_date":str(e["_date"]),
                 ...
                 "evidence_timestamp":str(arow["_ts"]),"reached_0_5x":bool(e["_reached_0_5x"])}
