...
def discover_sector_snapshots(
    ...
    # Sector Analysis must consume the dedicated Sector Summary source family.
    # Do NOT recursively inspect every intraday workbook: that creates a large,
    # unnecessary dependency on unrelated report families.
    sector_tokens = ("sector_summary", "sector summary", "sector-summary")
    candidates = sorted(
        [
            p for p in root.rglob("*")
            if p.is_file()
            and p.suffix.lower() in {".xlsx", ".xlsm", ".csv"}
            and any(token in p.name.lower() for token in sector_tokens)
        ],
        key=lambda p: (observation_time(p), str(p)),
    )
    ...
    for index, path in enumerate(candidates, start=1):
        ...
        for sheet, frame in frames:
            ...
            # A sheet is considered sector-capable only when a sector-like
            # column exists. No synthetic sector classification is created.
            if sector_col:
                work = frame.copy()
...
def aggregate_sector_rows(snapshot: SectorSnapshot) -> pd.DataFrame:
    ...
    symbol_col = snapshot.symbol_column
    ...
    frame = frame[frame["sector"].notna() & (frame["sector"] != "")].copy()
    if frame.empty:
        return frame

    aliases = {
        "price_chg": ["Price Chg %", "Price Chg", "Change %", "Change", "Pct Change", "% Change"],
        "oi_chg": ["OI Chg %", "OI Chg", "OI Change %", "OI Change"],
        "pcr_chg": ["PCR Chg %", "PCR Chg", "PCR Change %", "PCR Change"],
        "iv_chg": ["IV Chg %", "IV Chg", "IV Change %", "IV Change"],
        "ce_oi_chg": ["Tot CE OI Chg %", "Tot CE OI Chg", "CE OI Chg %", "CE OI Chg"],
        "pe_oi_chg": ["Tot PE OI Chg %", "Tot PE OI Chg", "PE OI Chg %", "PE OI Chg"],
        "pe_ce_oi_chg": ["Tot PE-CE OI Chg", "PE-CE OI Chg", "PE CE OI Chg"],
        "straddle": ["ATM Straddle %", "ATM Straddle", "Straddle %", "Straddle"],
    }
    for target, candidates in aliases.items():
        col = _find_col(frame.columns, candidates)
        if col and target not in frame.columns:
            frame[target] = pd.to_numeric(frame[col], errors="coerce")

    numeric = [
        "price_chg", "oi_chg", "pcr_chg", "iv_chg",
        "ce_oi_chg", "pe_oi_chg", "pe_ce_oi_chg", "straddle",
    ]
    for col in numeric:
