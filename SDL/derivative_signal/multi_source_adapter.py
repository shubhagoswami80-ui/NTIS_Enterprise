...
def _order_key(path: Path):
    try:
        # Windows CreationTime is the frozen chronology authority for SDL.
        return (path.stat().st_ctime_ns, path.name.lower())
    except OSError:
...
def discover_sources(
    ...
    if cutoff_path is not None:
        try:
            cutoff_ns = Path(cutoff_path).stat().st_ctime_ns
        except OSError:
            cutoff_ns = None
    ...
    def usable(p: Path) -> bool:
        if cutoff_ns is None:
            return True
        try:
            return p.resolve() == Path(cutoff_path).resolve() or p.stat().st_ctime_ns <= cutoff_ns
        except OSError:
            return False

    scoped_files=[p for p in files if usable(p)]

    base_candidates=sorted(
        [p for p in scoped_files if _family_match(p,SPECS[0])],
        key=_order_key,
    )
    b.base_history=base_candidates

    if cutoff_path is not None:
        cutoff_resolved=Path(cutoff_path).resolve()
        if cutoff_resolved.exists() and _family_match(cutoff_resolved,SPECS[0]):
            b.files["BASE"]=cutoff_resolved
    ...
    for spec in SPECS[1:]:
        candidates=[p for p in scoped_files if _family_match(p,spec)]
        if candidates:
    ...
    if bundle.base_history:
        try:
            ...
            bundle.errors.append(
                f"BASE opening snapshot: {type(exc).__name__}: {exc}"
            )
    return bundle
